let main={theme:'lite',version:'0.804',timePrev:performance.now(),searchScript:'no',searchStatus:'no',searchData:'',user:'',stamp:'',loginQ:false}
let menuVis=false
let searchVis=false
...
function zuserGet(){let user=localStorage.getItem(`user.user`)
if(user==null)return ''
...
until.setTime(until.getTime()+8*24*60*60*1000)
let now=new Date()
...
let links=[['index.htm','Home',0],['algebra/index.html','Algebra',0],['calculus/index.html','Calculus',1],['data/index.html','Data',0],['geometry/index.html','Geometry',0],['money/index.html','Money',1],['numbers/index.html','Numbers',1],['physics/index.html','Physics',0],['activity/index.html','Activities',1],['definitions/index.html','Dictionary',0],['games/index.html','Games',0],['puzzles/index.html','Puzzles',0],['worksheets/index.php','Worksheets',1],]
let s=''
...
s+='\n'}}
s+='</ul>'}
if(typ==1){s+='<ul role="list" class="bga1" style="display:inline-block; padding:6px; border-radius:10px; border: 1px solid; z-index:2; box-shadow: 3px 2px; ">'
let ht=36
...
s+='</li>'
s+='</ul>'}
if(typ==2){s+='<div id="login" style="display: inline-block; vertical-align:top; text-align:center; line-height:110%; padding:4px;">'
s+='</div>'
s+=' '
...
s+='</a>'
s+='<div id="pop" class="pop" style="position:absolute; width:300px; left:200px; opacity:0;"></div>'}
return s}
...
let browser=(function(agent){switch(true){case agent.indexOf('edge')>-1:return 'EdgeOld'
...
default:return 'Other'}})(window.navigator.userAgent.toLowerCase())
let os=''
...
main.timePrev=now}
function loginInfo(){let div=document.getElementById('pop')
let s=''
s+='Hi '+loginName()
s+='<br>'
s+='<button onclick="loginOut()" style="z-index:2;" class="btn" >Logout</button>'
s+='<div style="position:relative; margin:auto; text-align:right; font:14px Arial;  margin: 5px 0 1px 0; padding: 5px 0 5px 0;">'
s+='Password: '
s+='<input id="passOld" style="width: 120px; border-radius: 10px; font: 18px Arial; background-color: #eeffee; text-align:center;" value="'+''+'" />'
s+='<br>'
s+='New Password: '
s+='<input id="passNew" style="width: 120px; border-radius: 10px; font: 18px Arial; background-color: #eeffee; text-align:center;" value="'+''+'" />'
s+='<button onclick="loginPassChg()" style="z-index:2;" class="btn" >Change Password</button>'
s+='</div>'
s+='<div id="loginMsg" style="width:95%; margin: 0 0 5px 10px;">&nbsp;'
s+='</div>'
s+='<button onclick="loginHide()" style="z-index:2;" class="btn" >Close</button>'
div.innerHTML=s
div.style.left='-250px'
div.style.opacity=1}
function loginName(){let user=localStorage.getItem(`user.user`)
if(user==null)return ''
...
let user=loginName()
let s=''
if(user.length>0){s+='<a href="javascript:loginInfo()" style="aria-label="Logout">'
s+=user
s+='</a>'}else{s+='<a href="javascript:loginShow()" style="aria-label="Login">'
s+='Login'
s+='</a>'}
let div=document.getElementById('login')
div.innerHTML=s
console.log('loginPrompt',s)}
function loginShow(){let div=document.getElementById('pop')
div.innerHTML=loginHTML()
div.style.left='-250px'
div.style.opacity=1
document.getElementById('user').value=localStorage.getItem('user.user')}
function loginGo(){let user=document.getElementById('user').value
localStorage.setItem('user.user',user)
let pass=document.getElementById('pass').value
console.log('loginGo',user,pass)
loginMsg()
main.sock.login(user,pass)}
function loginNew(){let user=document.getElementById('user').value
localStorage.setItem('user.user',user)
let pass=document.getElementById('pass').value
console.log('loginNew',user,pass)
loginMsg()
if(user.length<6){loginMsg('Username: 6 or more characters')
return}
if(user.length>20){loginMsg('Username: 20 or fewer characters')
return}
if(pass.length<6){loginMsg('Password: 6 or more characters')
return}
main.sock.loginNew(user,pass)}
function loginPassChg(){let user=loginName()
...
if(passNew.length<6){loginMsg('Password: 6 or more characters')
return}
main.sock.loginPassChg(user,passOld,passNew)}
function loginOut(){localStorage.setItem('user.stamp','')
loginPrompt()
loginHide()
if(typeof loginDo==='function')loginDo()}
function loginMsg(s='&nbsp;'){console.log('loginMsg',s)
let div=document.getElementById('loginMsg')
if(div!=null)div.innerHTML=s}
function loginHide(){let div=document.getElementById('pop')
div.style.left='200px'
div.style.opacity=0}
function loginHTML(){let s=''
let user=''
let pass=''
s+='<div style="position:relative; margin:auto; text-align:center; font:14px Arial;  margin: 5px 0 1px 0; padding: 5px 0 5px 0;">'
s+='Username: '
s+='<input id="user" style="width: 120px; border-radius: 10px; font: 18px Arial; background-color: #eeffee; text-align:center;" value="'+user+'" />'
s+='<br>'
s+='Password: '
s+='<input id="pass" style="width: 120px; border-radius: 10px; font: 18px Arial; background-color: #eeffee; text-align:center;" value="'+pass+'" />'
s+='</div>'
...
s+='<button onclick="loginHide()" style="z-index:2;" class="btn" >Close</button>'
return s}
class MainSock{constructor(){this.socket=null
this.queue=[]
this.lines=[]}
send(msg){if(this.socket==null){this.queue.push(msg)
this.connect()}
if(this.socket.readyState===1){this.socket.send(msg)}}
waitForConnection(callback,interval){if(this.socket.readyState===1){callback()}else{let that=this
interval*=2
setTimeout(function(){that.waitForConnection(callback,interval)},interval)}}
connect(){console.trace('connect',1)
let that=this
let socket=new WebSocket('wss://www.mathbro.com:3003/echo')
socket.onopen=function(ev){console.log('[open] Connection established')
console.log('Sending to server')
for(let i=0;i<that.queue.length;i++){let msg=that.queue[i]
socket.send(msg)}}
socket.onclose=function(ev){console.log(ev.wasClean?'Disconnected':'Connection break: '+ev.code+' '+ev.reason)}
socket.onmessage=function(ev){let data={}
...
switch(data.msg){case 'usergood':this.histAdd('Login success!')
this.userSet('stamp',data.stamp)
this.userSet('stampWhen',new Date())
loginPrompt()
loginHide()
break
case 'usernew':this.histAdd('New User success!')
this.userSet('stamp',data.stamp)
loginPrompt()
loginHide()
break
case 'userchg':this.userSet('stamp',data.stamp)
loginMsg('Change successful!')
break
case 'usertaken':loginMsg('User taken')
this.userSet('stamp','')
break
case 'userinvalid':case 'passinvalid':loginMsg('User or Password invalid')
this.userSet('stamp','')
break
case 'userfail':loginMsg('Incorrect name or password')
this.userSet('stamp','')
break
case 'usernot':loginMsg('No such user')
break
default:}
if(typeof mainMsg==='function')mainMsg(data)}
userSet(name,val){localStorage.setItem(`user.${name}`,val)
console.log('optSet',name,val)}
histAdd(s){console.log('hist',s)
this.lines=this.lines.slice(Math.max(this.lines.length-4,0))
this.lines.push(s)}
login(user,pass){let data={msg:'login',name:user,pass:pass}
this.send(JSON.stringify(data))
this.histAdd('login: '+JSON.stringify(data))}
loginNew(user,pass){let data={msg:'loginNew',name:user,pass:pass}
this.send(JSON.stringify(data))
this.histAdd('loginNew: '+JSON.stringify(data))}
loginPassChg(user,passOld,passNew){let data={msg:'loginPassChg',name:user,passOld:passOld,passNew,passNew}
this.send(JSON.stringify(data))
console.log('loginPassChg: '+JSON.stringify(data))}
passHash(pass){return '#'+pass+'#'}}
themeSet(themeGet())
